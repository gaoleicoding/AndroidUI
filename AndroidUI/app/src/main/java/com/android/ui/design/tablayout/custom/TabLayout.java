...
public class TabLayout extends HorizontalScrollView {
  ...
  float tabTextSize;
  float tabSelectedSize = -1;
  float tabTextMultiLineSize;

  int textSelectedAnimationColor = 0;
  int textDefaultAnimationColor;
  int iconSelectedAnimationColor = 0;
  int iconDefaultAnimationColor;

  int tabIconWidth = 0;
  int tabIconHeight = 0;
  float tabIconScale = -1;

  int tabBackgroundResId;
  int backgroundAnimationColor = 0;
  int backgroundSelectedAnimationColor = 0;
  ...
  public TabLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    if (a.hasValue(R.styleable.TabLayout_tabTextSize)){
      tabTextSize = a.getDimensionPixelSize(R.styleable.TabLayout_tabTextSize,0);
      tabSelectedSize = tabTextSize;
    }
    if (a.hasValue(R.styleable.TabLayout_tabSelectedTextSize)){
      tabSelectedSize = a.getDimensionPixelSize(R.styleable.TabLayout_tabSelectedTextSize,0);
    }
    ...
    if (a.hasValue(R.styleable.TabLayout_tabTextColor)) {
      ...
      tabTextColors =
        ...
        textDefaultAnimationColor =
                tabTextColors != null ? tabTextColors.getDefaultColor() : Color.GRAY;
    }
    ...
    if (a.hasValue(R.styleable.TabLayout_tabSelectedTextColor)) {
      ...
      textDefaultAnimationColor = tabTextColors.getDefaultColor();
      textSelectedAnimationColor = selected;
    }

    if (a.hasValue(R.styleable.TabLayout_tabIconTint)) {
      tabIconTint =
              MaterialResources.getColorStateList(context, a, R.styleable.TabLayout_tabIconTint);
        iconSelectedAnimationColor = iconDefaultAnimationColor =
                tabIconTint != null?tabIconTint.getDefaultColor():Color.GRAY;
    }
    if (a.hasValue(R.styleable.TabLayout_tabSelectedIconTint)){
      final int selected = a.getColor(R.styleable.TabLayout_tabSelectedIconTint,0);
      tabIconTint = createColorStateList(tabIconTint.getDefaultColor(),selected);
      iconDefaultAnimationColor = tabIconTint.getDefaultColor();
      iconSelectedAnimationColor = selected;
    }
...
//    unboundedRipple = a.getBoolean(R.styleable.TabLayout_tabUnboundedRipple, false);
    ...
    if (a.hasValue(R.styleable.TabLayout_tabIconWidth)){
      tabIconWidth = a.getLayoutDimension(R.styleable.TabLayout_tabIconWidth, -2);
    }
    if (a.hasValue(R.styleable.TabLayout_tabIconHeight)){
      tabIconHeight = a.getLayoutDimension(R.styleable.TabLayout_tabIconHeight,-2);
    }
    if (a.hasValue(R.styleable.TabLayout_tabIconSize)){
      tabIconWidth = tabIconHeight = a.getLayoutDimension(R.styleable.TabLayout_tabIconSize,-2);
    }
    if (a.hasValue(R.styleable.TabLayout_tabIconScale)){
      tabIconScale = a.getFloat(R.styleable.TabLayout_tabIconScale,1);
    }

    if (a.hasValue(R.styleable.TabLayout_tabItemBackground)){
      backgroundAnimationColor = a.getColor(R.styleable.TabLayout_tabItemBackground, 0);
      tabBackgroundResId = 0;
    }
    if (a.hasValue(R.styleable.TabLayout_tabItemSelectedBackground)){
      backgroundSelectedAnimationColor = a.getColor(R.styleable.TabLayout_tabItemSelectedBackground, 0);
      tabBackgroundResId = 0;
    }
  ...
  private float nextPositionOffset = 0;
  /**
   * Set the scroll position of the tabs. This is useful for when the tabs are being displayed as
   * part of a scrolling container such as {@link ViewPager}.
   *
...
//    final View custom = tabLayout.getTabAt(position) != null ? tab.getCustomView() : null;
  ...
  private void updateAllTabs() {
    Log.i("fredZhu","updateAllTabs ======================================");
    for (int i = 0, z = tabs.size(); i < z; i++) {
      tabs.get(i).updateView();
    }
    Log.i("fredZhu","updateAllTabs end ======================================");
  }
  ...
  public void selectTab(@Nullable final Tab tab, boolean updateIndicator) {
    ...
    if (currentTab == tab) {
      ...
      if (tab != null) {
        ...
        float scale = tab.view.getCalculationScale(0);
        if (scale >= 0) {
          tab.view.textView.setScaleY(scale);
          tab.view.textView.setScaleX(scale);
        }
        float iconScale = tab.view.getCalculationIconScale(0);
        if (iconScale >= 0){
          tab.view.iconView.setScaleX(iconScale);
          tab.view.iconView.setScaleY(iconScale);
        }
      }
  ...
  public static class Tab {
    ...
    public Tab setCustomView(@Nullable View view) {
      customView = view;
      Log.i("fredZhu","setCustomView updateView");
      updateView();
      return this;
    }
    ...
    public Tab setIcon(@Nullable Drawable icon) {
      ...
      Log.i("fredZhu","setIcon updateView");
      updateView();
    ...
    public Tab setText(@Nullable CharSequence text) {
      ...
      if (text != null && text.length() > 0) {
        view.initViewPadding(text.toString());
      }
      updateView();
      return this;
    }
    ...
    public Tab setTabLabelVisibility(@LabelVisibility int mode) {
      ...
      Log.i("fredZhu","setTabLabelVisisbility updateView");
      this.updateView();
      if (BadgeUtils.USE_COMPAT_PARENT
          && view.hasBadgeDrawable()
          && view.badgeDrawable.isVisible()) {
    ...
    public Tab setContentDescription(@Nullable CharSequence contentDesc) {
      ...
      Log.i("fredZhu","setContentDescription updateView");
      updateView();
      return this;
    }
    ...
    void updateView() {
      if (view != null) {
        Log.i("fredZhu","updateView update");
        view.update();
      }
    }
...
//    /**
//     * 使用setTextSize 会导致很明显的文字抖动，使用setScaleX/Y效果好很多
//     */
//    void setPositionFromTabPosition(int position, float positionOffset) {
//      // positionOffset = 0/1 可以认为动画结束
//      Log.i("fredZhu", "text动画准备阶段 position:" + position + "  offset:" + positionOffset);
//      final View custom = getCustomView();
//      if (custom != null) {
//        return;
//      }
...
//        updateView();
//      } else {
//        animator(positionOffset);
//      }
//    }
//
//    void animateFromTabPosition(boolean isSelected, int duration){
//      if (animator != null && animator.isRunning()) {
//        animator.cancel();
//      }
//      updateOrRecreateAnimation(true,isSelected,duration);
//    }
//
//    private void updateOrRecreateAnimation(
//            boolean recreateAnimation, final boolean isSelected, int duration){
//
...
//    private void animator(float positionOffset){
//      if (view.textView != null) {
//        TextView tv = view.textView;
//        float newTextScale = view.getCalculationScale(positionOffset);
//        if (newTextScale >= 0) {
//          tv.setScaleX(newTextScale);
//          tv.setScaleY(newTextScale);
//        }
//        int newColor = view.getCalculationTextColor(positionOffset);
//        if (newColor != -1) {
//          tv.setTextColor(newColor);
//        }
//      }
//      if (view.iconView != null) {
//        ImageView iv = view.iconView;
//        int newColor = view.getCalculationIconColor(positionOffset);
//        if (newColor != -1) {
//          final Drawable icon = (iv.getDrawable() != null)
//                  ? DrawableCompat.wrap(iv.getDrawable()).mutate()
//                  : null;
//          if (icon != null) {
//            DrawableCompat.setTintList(icon, createColorStateList(newColor, newColor));
//          }
//        }
//        float iconScale = view.getCalculationIconScale(positionOffset);
//        if (iconScale >= 0) {
//          iv.setScaleX(iconScale);
//          iv.setScaleY(iconScale);
//        }
//      }
//    }
  ...
  public final class TabView extends LinearLayout {
    ...
    private void updateBackgroundDrawable(Context context) {
      ...
      } else if (backgroundAnimationColor != 0 || backgroundSelectedAnimationColor != 0){
        GradientDrawable temp = new GradientDrawable();
        int defaultColor = backgroundAnimationColor != 0?backgroundAnimationColor:Color.TRANSPARENT;
        int selected = backgroundSelectedAnimationColor != 0?backgroundSelectedAnimationColor:Color.TRANSPARENT;
        ColorStateList csl = createColorStateList(defaultColor, selected);
        temp.setColor(csl);
        baseBackgroundDrawable = temp;
      }else{
    ...
    private void drawBackground(@NonNull Canvas canvas) {
      Log.i("fredzhu123","drawBackGround  这里需要判断是否正在背景动画:"+(baseBackgroundDrawable == null));
...
//        // animation's updateListener to pick up the new target positions.
        updateOrRecreateAnimation(
                /* recreateAnimation= */ false, false, /* duration= */ -1);
      }
//      else {
...
//      }
    ...
    private int getCalculationColor(float fraction,@ColorInt int selectColor,@ColorInt int defaultColor) {
      if (selectColor == 0){
        return -1;
      }
      ...
      if (tabTextColors != null) {
        return (int) (argbEvaluator.evaluate(fraction, selectColor, defaultColor));
      }else{
        return -1;
      }
    }

    int getCalculationTextColor(float fraction){
      return getCalculationColor(fraction,textSelectedAnimationColor,textDefaultAnimationColor);
    }

    int getCalculationIconColor(float fraction){
      return getCalculationColor(fraction,iconSelectedAnimationColor,iconDefaultAnimationColor);
    }

    int getCalculationBackgroundColor(float fraction){
      return getCalculationColor(fraction,backgroundSelectedAnimationColor,backgroundAnimationColor);
    }

    float getCalculationScale(float fraction){
      if (tabSelectedSize <= 0){
        return -1;
      }
      return tabSelectedSize/tabTextSize - fraction*(tabSelectedSize/tabTextSize - 1);
    }

    float getCalculationIconScale(float fraction){
      if (tabIconHeight <= 0 || tabIconWidth <= 0 || tabIconScale <= 0){
        return -1;
      }
      return tabIconScale - fraction*(tabIconScale - 1);
    }

    //为缩放预留空间
    void initViewPadding(String text){
      if (tabSelectedSize >0 ) {
        Paint paint = textView.getPaint();
        paint.setTextSize(tabTextSize);
        float width1 = paint.measureText(text);
        Paint.FontMetricsInt fmi = paint.getFontMetricsInt();
        float height1 = fmi.bottom - fmi.top;
        paint.setTextSize(tabSelectedSize);
        float width2 = paint.measureText(text);
        fmi = paint.getFontMetricsInt();
        float height2 = fmi.bottom - fmi.top;
        int width = (int) ((width2 - width1) / 2 + 0.5f);
        int height = (int) ((height2 - height1) / 2 + 0.5f);
    ...
    void setPositionFromTabPosition(int position, float positionOffset) {
      ...
      // positionOffset = 0/1 可以认为动画结束
      final View custom = customView;
      if (custom != null) {
        return;
      }
      if (positionOffset == 0 || positionOffset == 1) {
        update();
        Log.i("fredzhu123", "动画结束");
...
//                    drawableStateChanged();
    ...
    private void animator(float positionOffset){
      if (textView != null) {
        ...
        float newTextScale = getCalculationScale(positionOffset);
        if (newTextScale >= 0) {
          tv.setScaleX(newTextScale);
          tv.setScaleY(newTextScale);
        }
        int newColor = getCalculationTextColor(positionOffset);
        if (newColor != -1) {
          tv.setTextColor(newColor);
        }
      }
      if (iconView != null) {
        ImageView iv = iconView;
        int newColor = getCalculationIconColor(positionOffset);
        if (newColor != -1) {
          final Drawable icon = (iv.getDrawable() != null)
                  ? DrawableCompat.wrap(iv.getDrawable()).mutate()
                  : null;
        ...
        float iconScale = getCalculationIconScale(positionOffset);
        if (iconScale >= 0) {
          iv.setScaleX(iconScale);
          iv.setScaleY(iconScale);
        }
      }
      if (backgroundAnimationColor != 0 &&
              backgroundAnimationColor != backgroundSelectedAnimationColor){
        int newColor = getCalculationBackgroundColor(positionOffset);
        if (newColor != -1){
          Drawable background = getBackground();
          if (!(background instanceof GradientDrawable)) {
            background = new GradientDrawable();
          }
          ((GradientDrawable) background).setColor(createColorStateList(newColor,newColor));
          ViewCompat.setBackground(this, background);
        }
      }
    }
...
//      int contentRightBounds = tabViewCenter + (tabViewContentWidth / 2);
      int tabViewWidth = tabView.getRight() - tabView.getLeft();
      int contentLeftBounds = tabViewCenter - (tabViewWidth / 4);
      int contentRightBounds = tabViewCenter + (tabViewWidth / 4);
